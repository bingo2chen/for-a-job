...
- 在单向数据流中，当你需要修改状态
### 20. Mutation为什么不能放异步，完全重新开始走一个修改的流程。这限制了状态修改的方式，让状态变得可预测，容易调试。
- 每个mutation执行完成后都会对应到一个新的状态变更，这样devtools就可以打个快照存下来，然后就可以实现 time-travel 了。如果mutation支持异步操作，就没有办法知道状态是何时更新的，无法很好的进行状态的追踪，给调试带来困难。
### 21. style 中 scope 设置原理 和 data-v-xxxx生成原理
- data-v-be498722：会自动在该组件所有标签上添加以data-v开头的特性名，只有子组件的根节点标签会添加父组件以及子组件的以data-v开头的特性，样式会受到两个组件的影响
- 生成原理：data-v 和 genId 拼成的，genId hash算出
### 22. $nextTick 实现原理
- 内部运行过程：调用$nextTick，会添加回调到任务队列中延迟执行，判断本轮事件中是否是第一次使用nextTick？是的话，向任务队列中添加队列然后结束，否则直接结束；
- 任务被执行->依次执行callbacks里面回调->清空callback
### 23. require/exports和import/export区别
- 出现的时间、地点不一样
  + require: 2009 Commonjs
  + import: 2015 ES6
- 不同端
  + require：只在node环境
  + import：node v9以上，浏览器大部分支持
...
- 用法不一致
- 参考链接：https://zhuanlan.zhihu.com/p/121770261
